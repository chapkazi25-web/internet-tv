...
What it does:
  ...
  4. Points the #EXTM3U url-tvg header at the raw EPG URL (no EPG
     ...
     (+ optional per-category split files).
  5. Optionally probes every stream URL (--check-streams) and writes
     README.md with active/dead counts plus a dead-channel table
     (same style as https://github.com/doms9/iptv).

Stdlib only. Usage:
  python3 update_playlist.py [--out-dir ./output] [--split] [--check-streams]
"""
...
def write_readme(path: Path, entries: list[dict], checked: bool,
                 m3u_url: str, epg_url: str, out: Path) -> Path:
    """Write README.md with active/dead counts + dead-channel table (doms9/iptv style)."""
    ...
    lines += [
        ...
        "",
        "```",
        f"{out / PLAYLIST_FILENAME}",
        "```",
        "",
        "### Per-category playlists",
        "",
        "```",
        f"{out / CATEGORIES_DIRNAME}/<category>.m3u",
        "```",
        "",
        "| Category | Channels |",
        "| --- | ---: |",
...
def main() -> int:
    ...
    ap.add_argument("--split", action="store_true", help="Also write one .m3u per category in <out-dir>/categories/")
    ap.add_argument("--no-renumber", action="store_true", help="Keep original tvg-chno values")
    ap.add_argument("--skip-download", action="store_true", help="Reuse existing source file in out-dir")
    ...
    ap.add_argument("--check-streams", action="store_true",
                    help="Probe every stream URL and include active/dead stats in README.md")
    ...
    ap.add_argument("--readme", default=README_FILENAME,
                    help="Where to write README.md (default: README.md in current directory)")
    args = ap.parse_args()

    out = Path(args.out_dir)
    out.mkdir(parents=True, exist_ok=True)
    raw_m3u = out / SOURCE_FILENAME
    ...
    org_path = out / PLAYLIST_FILENAME
    ...
    readme_path = write_readme(Path(args.readme), entries, args.check_streams,
                               args.m3u_url, args.epg_url, out)
    print(f"Wrote {readme_path}")

    if args.split:
        cdir = out / CATEGORIES_DIRNAME
        cdir.mkdir(exist_ok=True)
        # remove stale files from previous naming schemes
        for stale in cdir.glob("*.m3u"):
            stale.unlink()
        for cat in counts:
            p = cdir / f"{slugify_category(cat)}.m3u"
            with open(p, "w", encoding="utf-8") as f:
                ...
                for e in (x for x in entries if x["category"] == cat):
                    ...
                    for x in e["extra"]:
                        f.write(x + "\n")
                    f.write(e["url"] + "\n")
        print(f"Wrote {len(counts)} per-category files to {cdir}/")

    print(f"\nDone. EPG is referenced raw via url-tvg={args.epg_url}")
    print(f"Load {out / PLAYLIST_FILENAME} in your IPTV player (e.g. TiviMate).")
    return 0
